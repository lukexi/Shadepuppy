...
float posToFloat( vec3 p ){
    ...
    float f = triNoise3D( p * 3. + vec3(  iGlobalTime * .005 ), .1 );
    return f;
...
vec2 map( vec3 pos ){  
    ...
    vec2 res = vec2( rotatedBox( pos , vec3( iGlobalTime * .05 + 1., iGlobalTime * .02 + 2. , iGlobalTime * .03  ), vec3( 0.1 ) , .03 ) , 1.0 );
...
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir )
{
    ...
    vec3 col = render( fragRayOri + vec3(0.0,0.0,.3), fragRayDir );
